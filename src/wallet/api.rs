...
impl Wallet {
    ...
    pub(super) fn import_addresses(
        ...
        let import_requests = hd_descriptors
            ...
            .chain(
                swapcoin_descriptors.iter().map(|desc| ImportMultiRequest {
                    timestamp: Timestamp::Now,
                    descriptor: Some(desc),
                    watchonly: Some(true),
                    label: Some(&address_label),
            ...
            .chain(
                contract_scriptpubkeys.iter().map(|spk| ImportMultiRequest {
                    timestamp: Timestamp::Now,
            ...
            .chain(
                self.store.fidelity_bond.iter().map(|(_, (_, spk, _))| ImportMultiRequest {
                    timestamp: Timestamp::Now,
                    script_pubkey: Some(ImportMultiRequestScriptPubkey::Script(spk)),
                    watchonly: Some(true),
                    label: Some(&address_label),
    ...
    pub fn lock_unspendable_utxos(&self) -> Result<(), WalletError> {
        ...
        self.rpc.call::<Value>("lockunspent", &[Value::Bool(true)])?;
